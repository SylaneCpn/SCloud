            ...
            if let Some(idx) = path.rfind('.') {
                ...
                match content_ext {
                    ...
                    "txt" | "html" | "css" | "js" | "rs" => {
                        ...
                        return Ok(([(header::CONTENT_TYPE,&content_type )], String::from_utf8_lossy(&contents).to_string()).into_response());
                    }
                    ...
                    _ => { return Ok(contents.into_response());}
                }
            }
...
async fn respond_dir(path : &str) -> io::Result<Response> {
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        if metadata.is_file() {
            ...
            fls.push(File {name : name.clone(), content_type : "file".to_string() , full_path : format!("{}/{}",path , name.clone())});
        }
        //directory
        else {
            let name = entry.file_name().into_string().unwrap();
